...
def add_input(text):
    console.print(Padding(f'{text}', {2, 3}))
...
def add_inputs(text):
    inputs = []
    console.print(Padding(f'''{text} If that's all, and you want me to start generating a response, just write: "ok":''', {2, 4}))
    while True:
...
def printout(the_world, chapters, definitions, substitutions):
    try:
        console.print(Padding(the_world, {2, 3}))
    except: 
        pass
    
    try:
        for key, value in chapters.items():
            console.print(Padding(f'{key.upper()} {value}', {2, 3}))
    except:
        pass
    
    try:
        console.print(Padding(f'{definitions} {substitutions}', {2, 3}))
    except:
        pass    


def rollback(memory):
    console.print(Padding('How far should I roll back?', {2, 3}))
    ...
    try:
        ...
        console.print(Padding('Sorry, no such world! Try again.', {2, 3}))
        return (False)
